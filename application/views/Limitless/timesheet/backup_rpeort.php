        ...
        $(".ymmd").change(function(){
            ...
            if((start == "" & end != "") || (start != "" & end == "")){
                ...
                $.ajax({
                    ...
                    type: 'POST',
                    dataType: 'json',
                    data : {start: start, end: end, pc_id : pc_id, project_id : project_id},
                    async: false,
                    success: function (res) {
                        ...
                        if(res.status == 'success'){
                            ...
                            $.ajax({
                                ...
                                success: function (res) {
                                    ...
                                    $.each(res, function( index, value ) {
                                        ...
                                        pa += "<th class = 'text-center'>Plan & Progress</th><th class = 'text-center'>Activity</th>"
                                    });

                                }
                            });
                            var tab = '<div class = "row table">'+
                            ...
                            $.each(res.data, function(index, val){
                                tab += '<tr>';
                                tab += '<td>'+value.fullname+'</td>';
                                plan = value.existing_plan;
                                    ...
                                    $.each(dt, function(index, date){
                                        ...
                                        if(idx < 0){
                                            ...
                                            $.each(ep[date], function(key, plan){
                                                ...
                                                tab += '<td>'+plan.plan+'</td>';
                                                tab += '<td>'+pm_detail.segment_name+'</td>';
                                                tab += '<td>'+pm_detail.span_hh_start+'-'+pm_detail.span_hh_end+'</td>';
                                                tab += '<td>'+pm_detail.target+'</td>';
                                                tab += '<td>'+pm_detail.uom+'</td>';
                                                tab += '</tr>';
                                            })
                                            tab += '</tbody>';
                                            tab += '</table>';
                            ...
                            // $.each(res.data, function( index, val ) {
                            //     tab += '<tr>';
                            //     tab += '<td>'+value.fullname+'<br/><button type = "button" class = "btn bg-teal-400 btn-sm create_plan" user_id = "'+value.user_id+'" user_name = "'+value.fullname+'" loc = "'+value.loc+'">Setting Plan</button></td>';
